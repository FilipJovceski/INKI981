...
char tocniPrvi[4] = { 'a', 'c', 'b', 'd' };
char tocniVtori[4] = { 'a', 'd', 'b', 'a' };
char tocniTreti[4][2] = { {'b', 'c'}, {'a', 'd'}, {'b', 'd'}, {'a', 'd'} };

// Funkcija za podatoci na studentot
...
// Funkcija za zapisuvanje odgovori
...
// Funkcija za proverka na odgovori i zacuvuvanje na istite
...
// Funkcija za poenite i dobivanje ocenka
...
int main() {
    ...
    // Sozdavanje text dokument za podatocite na studentot

    const string filename = lice.ime + lice.prezime + "INKI" + lice.indeks;
    ifstream fileExist(filename);

    if (fileExist.good()) {
        //proverka dali istiot student go resaval prethodno testot
        cout << "\nVekje ste go resavale testot! " << endl;
        exit(0);
    }
    else {
        //dokolku istiot student ne go resaval testot prethodno
        ...
        file.close();
        // Se prevzemaat prasanjata od prasanja.txt
